...
Set::Set() {
	head = new Node; // initalize head with a dummy node pointing to itself
...
Set::Set(const Set& other) {
	...
	Node* curr = head->next; // set iterator pointer to start of linked list
...
bool Set::erase(const ItemType& value) {
	...
	while (curr != head) {
		...
		curr = curr->next; // shift curr forward
	}
	return true;
}

bool Set::contains(const ItemType& value) const {
	Node* p = head->next; // set iterator to first node of the list
...
void unite(const Set& s1, const Set& s2, Set& result) {
	...
	temp = s1; // copy s1 into temporary set

	// copy s2 into temporary set
	for (int i = 0; i < s2.size(); i++) {
		ItemType item;
		s2.get(i, item); // get the item from s2 to insert
		temp.insert(item); // insert checks for duplicates, ony unique items are added
	}
	result = temp;
}
...
void butNot(const Set& s1, const Set& s2, Set& result) {
	...
	for (int i = 0; i < s1.size(); i++) {
		ItemType item;
		s1.get(i, item);
		if (!s2.contains(item)) { // only insert into temp if item from s1 is not in s2
			temp.insert(item);
		}
	}
	result = temp;
