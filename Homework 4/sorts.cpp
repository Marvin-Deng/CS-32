...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (int i = 1; i < s.size(); i++) {
        Student toBeInserted = s[i];
        int idx = i;
        // if the number in the vector is less than the target value, insert position found
        while (idx > 0 && comp(toBeInserted, s[idx - 1])) {
            s[idx] = s[idx - 1]; // shift Students to the right
            idx--;
        }
        s[idx] = toBeInserted; // set target value to the current position
    }
}
...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    // 1. Create an auxiliary copy of students to facilitate the later reordering.
    //    Create a vector of Student pointers, and set each pointer to point to the corresponding Student in auxStudents.
    // 2. Sort the vector of pointers using the STL sort algorithm
    //    with the comp parameter as the ordering relationship.
    // 3. Using the now-sorted vector of pointers, replace each Student
    ...
    vector<Student*> stuPointers;
    for (int i = 0; i < auxStudents.size(); i++) {
        stuPointers.push_back(&(auxStudents[i]));
    }

    sort(stuPointers.begin(), stuPointers.end(), comp);

    for (int i = 0; i < stuPointers.size(); i++) {
        students[i] = *stuPointers[i];
    }
